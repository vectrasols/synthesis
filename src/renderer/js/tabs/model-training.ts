...
const ModelTab = (() => {
  ...
  function isClassificationTarget(col) {
    if (!col || uniqueCount(col) < 2) return false;
  ...
  function minRowsForModel(modelType) {
    if (modelType === 'stacking_regression' || modelType === 'stacking_classification') return 5;
    if (SEMI_SUPERVISED_MODELS.has(modelType)) return 6;
    if (modelType === 'dbscan') return 3;
    if (modelType === 'isolation_forest' || modelType === 'one_class_svm') return 4;
    if (UNSUPERVISED_MODELS.has(modelType)) return 2;
    return 3;
  }

  function minNumericFeaturesForModel(modelType) {
    if (modelType === 'pca_projection' || modelType === 'ica_projection') return 1;
    return 1;
  }
  ...
  function modelGeneralRule(modelType) {
    if (!hasData()) return unavailable('load data first');
    const minRows = minRowsForModel(modelType);
    if (_rows < minRows) return unavailable(`needs ${minRows}+ rows`);

    if (REGRESSION_MODELS.has(modelType)) {
      if (!validRegressionTargets().length) return unavailable('needs numeric target plus numeric features');
      return available();
    }

    if (CLASSIFICATION_MODELS.has(modelType)) {
      if (!validClassificationTargets().length) return unavailable('needs class target plus numeric features');
      return available();
    }
    ...
    if (numericFeatureCount() < minNumeric) return unavailable(`needs ${minNumeric}+ numeric feature`);
    return available();
  }

  function targetRule(modelType, col) {
    if (!hasData()) return unavailable('load data first');
    if (!col) return modelRequiresTarget(modelType) ? unavailable('select target column') : available();

    if (isRegressionModel(modelType)) {
      if (!isNumericCol(col)) return unavailable('regression target must be numeric');
      if (uniqueCount(col) < 2) return unavailable('target needs at least two values');
      if (numericFeatureCount(col) < 1) return unavailable('needs another numeric feature');
      return available();
    }

    if (isClassificationModel(modelType)) {
      if (!isClassificationTarget(col)) return unavailable('target should be categorical or low-cardinality');
      if (numericFeatureCount(col) < 1) return unavailable('needs numeric feature columns');
      return available();
    }

    const minNumeric = minNumericFeaturesForModel(modelType);
    if (numericFeatureCount(col) < minNumeric) return unavailable(`would leave fewer than ${minNumeric} numeric feature`);
    return available();
  }
  ...
  function updateTargetState() {
    ...
    if (labelEl) labelEl.textContent = requiresTarget ? 'Target Column' : 'Exclude Column';
    if (targetEl) {
      ...
      _cols.forEach(col => {
        ...
        if (rule.ok) hasAllowedTarget = true;
      });
