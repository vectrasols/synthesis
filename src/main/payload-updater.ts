...
type ActivePackage = PayloadPackage & {
  dir: string;
  installedAt: string;
};
...
type PayloadState = {
  version: string;
  platform: string;
  packages: Record<string, ActivePackage>;
  appliedAt?: string;
  stagedAt?: string;
};
...
function readBundledBaseline(): PayloadManifest | null {
  const baseline = readJsonFile(getBundledBaselinePath());
  if (!baseline || baseline.platform !== getPlatformKey()) return null;
  return baseline;
}

function readActiveState(): PayloadState | null {
  const state = readJsonFile(getStatePath()) as PayloadState | null;
  if (!state || state.platform !== getPlatformKey()) return null;
  if (compareVersions(state.version, app.getVersion()) < 0) return null;
  return state;
}

function getKnownPackage(pkg: PayloadPackage): PayloadPackage | ActivePackage | null {
...
async function applyPendingPayloadUpdate() {
  const pending = readJsonFile(getPendingPath()) as PayloadState | null;
...
function setupPayloadUpdater(mainWindow) {
  ...
  if (process.env.SYNTHESIS_DISABLE_PAYLOAD_UPDATES === '1') return;
...
async function checkForPayloadUpdates(send) {
  ...
  const manifest = await fetchJson(manifestUrl) as PayloadManifest;
  ...
  try {
    const stagedPackages: Record<string, ActivePackage> = {
...
function validateManifest(manifest: PayloadManifest) {
...
function resolvePackageUrl(manifestUrl, pkg: PayloadPackage) {
  if (pkg.url && /^https?:\/\//i.test(pkg.url)) return pkg.url;
...
function downloadBuffer(url, redirects = 0): Promise<Buffer> {
...
function downloadFile(url, filePath, onProgress, redirects = 0): Promise<void> {
  return new Promise((resolve, reject) => {
    const req = request(url, redirects, (res) => {
...
function sha512File(filePath): Promise<string> {
  return new Promise((resolve, reject) => {
...
async function extractTarGz(archivePath, destination) {
  ...
  while (offset + 512 <= archive.length) {
    ...
    if (type === '5') {
      ...
      if (process.platform !== 'win32') await fsp.chmod(targetPath, mode);
    }
