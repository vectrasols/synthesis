...
const CleanTab = (() => {
    ...
    function setSelectOptions(selectId, rows, placeholder = 'Select…') {
        ...
        rows.forEach(row => {
            ...
            if (row.ok && !firstAllowed)
                firstAllowed = row.value;
        });
    ...
    function updateCleaningAvailability() {
        ...
        if (outlierThreshold)
            outlierThreshold.disabled = !hasNumeric || !removeOutliers?.checked;
        ...
        updateMethodOptions('dtypeConvert', value => {
            ...
            if (!col)
                return { ok: false, reason: 'select a column first' };
            if (value === 'numeric' && isNumericCol(col))
                return { ok: false, reason: 'already numeric' };
            if (value === 'categorical' && isCategoricalCol(col))
                return { ok: false, reason: 'already categorical' };
            return { ok: true };
        });
        ...
        if (binarizeThreshold)
            binarizeThreshold.disabled = !document.getElementById('binarizeCol')?.value;
        ...
        if (selectionK)
            selectionK.disabled = !document.getElementById('selectionMethod')?.value;
        ...
        updateMethodOptions('extractionMethod', value => {
            if (!value)
                return { ok: true };
            if (!hasNumeric)
                return { ok: false, reason: 'needs numeric columns' };
            if (value === 'lda' && !document.getElementById('extractionTarget')?.value)
                return { ok: false, reason: 'select a target first' };
            return { ok: true };
        });
        const extractionComponents = document.getElementById('extractionComponents');
        if (extractionComponents)
            extractionComponents.disabled = !document.getElementById('extractionMethod')?.value;
    ...
    function renderTable(columns, rows) {
        ...
        body.innerHTML = rows.map(row => '<tr>' + row.map(v => `<td title="${v}">${v}</td>`).join('') + '</tr>').join('');
    }
